local protectkey = "Pizzagamer!23"

ESX = exports["es_extended"]:getSharedObject()

RegisterNetEvent('SC-AutoDiefstal:pay')
AddEventHandler('SC-AutoDiefstal:pay', function(bedrag, protect)
local src = source
local xPlayer = ESX.GetPlayerFromId(src)
        if protect == protectkey then
            xPlayer.addAccountMoney('black_money', bedrag)
		else
		DropPlayer(source, "HACKER | probeert geld intespawnen")
	end
end)
